...
description: Walks through adding a new skill or instruction rule to the APIHub agent-packages store or a consumer repository. Use when creating, extending, or publishing agent skills and rules for APIHub repositories.
---
...
|--------------|----------------------------------|------------------------------|
| **Skill** (workflow checklist) | `qubership-apihub-ci/agent-packages/skills/apihub-go-*` | `<repo>/agent-packages/skills/<name>/` |
| **Instruction** (glob rule) | `qubership-apihub-ci/.../go-conventions/` or `common-conventions/` | `<repo>/agent-packages/instructions/<group>/` |

- **Generic** content goes in the CI central store; consumers add a Git dependency in `apm.yml`.
- **Repo-specific** content stays in that repository's `agent-packages/` folder and is referenced
  with a relative path (e.g. `./agent-packages/skills/my-skill`).
...
Create a directory under the chosen `agent-packages/skills/<name>/`:
...
Create `agent-packages/instructions/<group>/`:
...
- **CI store packages:** add a row to `qubership-apihub-ci/agent-packages/README.md`.
- **Repo-local packages:** add a row to `<repo>/agent-packages/README.md`.

## 7. Consumer rollout

Add dependencies to root `apm.yml` (Git refs for CI store, `./agent-packages/...` for local)
and run:
...
Commit `apm.yml` + `apm.lock.yaml`; gitignore only `apm_modules/`.
Commit repo-local `agent-packages/` sources and deployed `.cursor/` / `.claude/` harness trees.
